...
public:
  ...
  void visualize(const vector<State>& path)
  {
    ...
    for (const auto& state : path)
    {
      ...
      costmap_2d::transformFootprint(state.x, state.y, state.theta, wheel_fl, oriented_wheel_fl);
      costmap_2d::transformFootprint(state.x, state.y, state.theta, wheel_fr, oriented_wheel_fr);
      costmap_2d::transformFootprint(state.x, state.y, state.theta, footprint_, oriented_footprint);
